...
export async function sslCheck(hostname: string, port = 443) {
      ...
      () => {
        ...
        resolve({
          ...
          daysRemaining: cert.valid_to
            ? Math.ceil((new Date(cert.valid_to).getTime() - Date.now()) / (1000 * 60 * 60 * 24))
            : null,
        });
      },
    );
...
export async function sslExpiry(hostname: string) {
  const result = (await sslCheck(hostname)) as {
    success: boolean;
    validTo?: string;
