...
export function ApiKeyManager({ id }: { id?: string }) {
  ...
  const createKey = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/v1/account/keys`, {
        ...
        body: JSON.stringify({ email, name, keyName, teamName: teamName || undefined }),
      });
    ...
    <Card id={id}>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Pass your key as <code className="rounded bg-muted px-1">X-API-Key</code> on API requests for
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2" id="create-key">
          <div className="space-y-2">
            ...
            <Input id="api-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
          </div>
          <div className="space-y-2">
        ...
        <Button type="button" onClick={createKey} disabled={loading || !email || !name}>
